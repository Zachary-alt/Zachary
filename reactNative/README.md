...
React Naitve的简介：Facebook在React.js Conf2015⼤会上推出的⼀个用于开发Android和iOS App的 
...
1. 跨平台开发：运用React Native，我们可以使用同⼀份业务逻辑核心代码来创建原⽣应用运行在Web端，Android端和iOS端； 
...
> 心得：⾼版本的模拟器通常没有菜单键的，不过Nexus S上是有菜单键的，如果想使用菜单键，可以创建⼀个Nexus S的模拟器。 
...
### WebView
 ...
 WebView 创建⼀个原⽣的 WebView，可以用于访问⼀个⽹⻚。
...
> 注意：新版的RN已经用react-naitve-webView替代RN原⽣Core中的WebView
...
经常使用ListView的同学都知道，这个组件的性能比较差，尤其是当有⼤量的数据需要展示的时候，ListView对内存的占用较多，常出现丢帧卡顿现象 
...
[FlatList](https://reactnative.cn/docs/flatlist) 和 [SectionList](https://reactnative.cn/docs/sectionlist) 的底层实现，VirtualizedList通过维护⼀个有限的渲染窗⼝(其中包含可见的元素)，并将渲染窗⼝之外的元素全部用合适的定⻓空⽩空间代替的方 式，极⼤的改善了内存使用，提⾼了⼤量数据情况下的渲染性能。这个渲染窗⼝能响应滚动行为，元素离可视区越远优先级越低，越近优先级越⾼，当用户滑动速度过快时，会出现短暂空⽩的情况。
