...
最经典和简单粗暴的排序算法，简而言之，就是挨个对比，如果比右边的数字大，就交换位置 遍历⼀次，最大的在最右边，重复步骤，完成排序
...
大概就是找⼀个标志位，先遍历⼀次，所有个头比他矮的，都站左边，比他个头高的，都站右边，遍历⼀次，就把数组分成两部分，然后两遍的数组，递归执行相同的逻辑
...
快排我们了解到，递归就是自己调用自己，形成⼀个调用栈，逐渐缩小目标，到达截止条件返回执行的 
...
哈希的问题也很明显，比如两个数的hash值⼀样的时候，会发生碰撞，可以用存储链表的方式来解决(重复的值存在链表里) 这些V8帮我们处理的很好了
...
动态规划是⼀种常见的「算法设计技巧」，并没有什么高深莫测，至于各种高大上的术语，那是吓唬别人用的，只要你亲自体验几把，这些名词的含义其实显而易见，再简单不过了。 
...
再举个找零的小栗子，:假如有 1, 5, 10, 20 ,50,100的人⺠币
